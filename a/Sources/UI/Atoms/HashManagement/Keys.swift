...
enum PublicKeyIdentity {
  static func publicKeyHex(from key: String) -> String? {
    let normalized = key
      .trimmingCharacters(in: .whitespacesAndNewlines)
      .lowercased()
    guard !normalized.isEmpty else { return nil }

    if let decoded = decode_bech32_key(normalized) {
      switch decoded {
      case .pub(let publicKeyHex):
        return publicKeyHex
      case .sec(let privateKeyHex):
  ...
  static func avatarKey(from key: String) -> String {
    let normalized = key
      .trimmingCharacters(in: .whitespacesAndNewlines)
      .lowercased()
    return publicKeyHex(from: normalized) ?? normalized
  }
}

func generate_new_keypair() -> Keypair {
  guard let key = try? secp256k1.Signing.PrivateKey() else {
    assertionFailure("Unable to generate secp256k1 private key")
