...
import SwiftUI
import KeychainSwift /// Keychain Swift Library for safe key storage
...
class KeyManager: ObservableObject {
    ...
    func isValidBech32EncodedKey(_ key: String) -> Bool {
        let keyTrimmed = key.replacingOccurrences(of: " ", with: "").lowercased() /// Delete blank spaces and lowercase the string before decoding
        if let _ = decode_bech32_key(keyTrimmed) {
            return true
        }
        else {
            return false
