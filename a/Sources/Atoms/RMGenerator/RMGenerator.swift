    ...
    var body: some View {
        if let url = url {
            ...
            if pubkey.range(of: pattern, options: .regularExpression) != nil {
                ...
                let head = "head-\(String(format: "%02d", RM[0] > 15 ? limitRM(RM[0], limit: 15) : RM[0])).png"
                let eyes = "eyes-\(String(format: "%02d", RM[1] > 11 ? limitRM(RM[1], limit: 11) : RM[1])).png"
                let nose = "nose-\(String(format: "%02d", RM[2] > 1 ? limitRM(RM[2], limit: 1) : RM[2])).png"
                let mouth = "mouth-\(String(format: "%02d", RM[3] > 9 ? limitRM(RM[3], limit: 9) : RM[3])).png"
                let hh = "hh-\(String(format: "%02d", RM[4] > 26 ? limitRM(RM[4], limit: 26) : RM[4])).png"
                let acc = "acc-\(String(format: "%02d", RM[5] > 17 ? limitRM(RM[5], limit: 17) : RM[5])).png"
