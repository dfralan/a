...
struct RootView: View {
    ...
    var body: some View {
        ...
        .preferredColorScheme(coordinator.themeMode > 0 ? coordinator.themeMode > 1 ? .dark : .light : .none)
...
enum SelectedView: String, Codable {
    case home, messages, notifications, keyManager, relays, wallet, settings, collaborate, logout, profileDetailed
    var title: String {
...
extension RootView {
    ...
    func link(to page: SelectedView) -> some View {
        ...
        case .home:
            ...
            return AnyView(NavigationLink(destination: detailContent(for: page)) {
                HStack {
                    AvatarView(size: 40)
                    VStack(alignment: .leading) {
